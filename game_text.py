...
class GameText:
    ...
    def __init__(self, settings, screen):
        ...
        self.game_over_image = self.settings.game_over_font.render(
            "Game  Over", True, self.settings.text_color, self.settings.bg_color
        )
        self.game_over_rect = self.game_over_image.get_rect()
        # Position on screen
        self.game_over_rect.center = self.screen.get_rect().center
        # Create a background rect for the game over text
        self.game_over_bg = pygame.Rect(
            0, 0, self.game_over_rect.width + 20, self.game_over_rect.height + 20
        )
        # Position on screen
